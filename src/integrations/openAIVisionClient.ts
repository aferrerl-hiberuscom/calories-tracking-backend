// OpenAI Vision client for quantity estimation — feature 007
// Calls OpenAI Vision API to estimate ingredient quantities from an image.
...
async function callOnce(
  ...
  if (!apiKey) {
    throw new VisionApiError("OPENAI_API_KEY not configured");
  }
  ...
  if (!res.ok) {
    throw new VisionApiError(`OpenAI API returned ${res.status}`);
  }
  ...
  try {
    ...
    throw new VisionApiError("OpenAI response is not valid JSON");
  }
...
export async function analyzePortions(
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      return await callOnce(imageUrl, ingredients);
    } catch (err) {
